...
Última actualización: 9 Abr 2026 — 10:00

# SESSION.md — EasyPro Notarial 2
> Última actualización: 9 Abr 2026 — 23:00

---

## OBJETIVO DEL PROYECTO

Software notarial SaaS multinotaría para digitalizar la operación documental de notarías colombianas. Go-live con Notaría de Caldas el **14 de abril de 2026**.
...
### Subdominios objetivo
- caldas.easypro.co → Notaría de Caldas (go-live 14 Abr)
...
- Repo: C:\EasyProNotarial-2\easypro2
- Rama: master
- Último commit: 56f2107 (+ cambios del 9 Abr sin commitear)
...
## CAMBIOS DEL 9 ABR 2026

### Landing page (`frontend/components/marketing/landing-page.tsx`)
- ✅ Sección **Casos de éxito** agregada entre Identidad Institucional y FAQs
  - Tarjeta: Notaría Única del Círculo de Caldas — badge "Live desde Abr 2026"
  - Tarjeta: Notaría Primera del Círculo de Bello — badge "Demo disponible"
  - Ambas con tags de actos, CTA → WhatsApp, hover verde
- ✅ **Modal de video testimonial** en cada tarjeta
  - Botón "Ver testimonio" con ícono Play
  - Modal con iframe YouTube embebido
  - URLs placeholder: `https://www.youtube.com/embed/TODO_CALDAS` y `TODO_BELLO`
  - Nota: "Video testimonial — próximamente disponible"
- ✅ Nombre navbar aumentado: `text-base font-black`

### Branding (`frontend/lib/branding.ts`)
- ✅ `legalName`: "Notaría 75 del Círculo de Bogotá" → **"Plataforma notarial digital"**
- ✅ `city`: "Bogotá D.C." → **"Colombia"**
- ✅ Subtítulo navbar: `text-xs font-medium uppercase tracking-[0.18em]`

### Previsualizador Documento Gari (`frontend/components/cases/case-detail-workspace.tsx`)
- ✅ Tab **"Documento Gari"** completamente implementado
- ✅ Toolbar: toggle "Ver borrador" / "Modo revisor" + botones Descargar Word y Regenerar con Gari
- ✅ Píldoras de versiones (v1, v2, v3...) con fecha
- ✅ **Hoja de papel** — render del documento con tipografía notarial (Georgia, justificado, negrillas en encabezados)
- ✅ Split por `[[--]]` o `\n` para separar párrafos correctamente
- ✅ **Panel revisor lateral** (aparece en Modo revisor):
  - Campo de instrucciones de corrección
  - Botón "Aplicar correcciones" → llama Gari con la nota
  - Historial de versiones con descarga individual
  - Botón "Aprobar documento"
- ✅ Estado vacío con CTA a tab Documentos cuando no hay borrador
- ✅ Fix: muestra hoja cuando hay versiones aunque `gari_draft_text` sea null en sesiones anteriores
- Estados nuevos: `reviewerMode`, `correctionNote`, `isRegenerating`

### Documento ejecutivo para socios
- ✅ `EasyPro2_Reunion_Socios_9Abr2026.docx` generado con:
  - Tabla de tipos de acto
  - Diagrama de flujo embebido como imagen PNG
  - Tabla de estado por paso (semáforo)
  - Catálogo de etiquetas por tipo de acto (Poder General, Venta sin RPH, Salida del País)
  - Tabla de inconsistencias detectadas en plantillas
  - Roadmap al 14 de abril
...
4. Características — 6 cards oscuras con íconos verdes
5. Identidad institucional — mock Notaría de Caldas
6. **Casos de éxito** ✅ NUEVO — Caldas (live) + Bello (demo) con modal video
7. FAQs — acordeón useState, 5 preguntas
8. CTA final — título gigante, botón verde
9. Footer suscripción — email + checkbox tratamiento de datos
10. Footer corporativo — logo, producto, empresa, Medellín Carrera 43B #1A Sur-70
11. Footer copyright — © 2026 EasyPro Notarial · Colombia
12. Botón flotante WhatsApp: https://wa.me/573107932844
13. Botón flotante Llamada: tel:+573107932844
...
- Layout dos columnas: imagen izquierda enmarcada en card / formulario derecha
- Imagen: /notario-hero.png enmarcada con bordes redondeados
- Badge "PLATAFORMA ACTIVA — Notaría de Caldas — Go-live 14 Abr 2026"
- Fondo formulario: #0D1B2A paleta Finaktiva
- Campos email y contraseña oscuros
- Checkbox tratamiento de datos obligatorio
- Checkbox recordar sesión
...
→ Gari redacta documento notarial completo en español jurídico colombiano
→ Se guarda como borrador Word versionado (v1, v2, v3...)
→ Aprobador revisa en tab "Documento Gari" — hoja de papel + modo revisor
→ Puede regenerar con correcciones (instrucción en texto libre)
→ Aprueba → Notario firma → PDF final
```

### Archivos creados/modificados
- `backend/app/services/gari_document_service.py` — servicio completo
- `backend/app/modules/document_flow/router.py` — endpoint POST generate-with-gari
- `frontend/components/cases/case-detail-workspace.tsx` — UI completa con previsualizador

### Configuración
- `backend/.env` — OPENAI_API_KEY configurada
- Modelo: gpt-4o, temperature: 0.2, max_tokens: 4000
- Separador de párrafos en texto Gari: `[[--]]`

### Prueba exitosa
- Caso CAS-2026-0003 (ID 22) — v1 a v5 generadas
- Poder General completo con cláusulas, constancias legales y liquidación
- Word descargado y verificado — tildes y ñ correctas

---

## PLANTILLAS WORD ETIQUETADAS ✅ RECIBIDAS

### Tipos de acto con plantillas listas
| Tipo | Variantes | Estado |
|---|---|---|
| Poder General | Notario Encargado | ✅ Lista |
| Venta sin RPH | Notario Encargado + Titular | ✅ Lista |
| Salida del País H→M | Notario Encargado + Titular | ✅ Lista |
| Salida del País M→H | Notario Encargado + Titular | ✅ Lista |

### Inconsistencias detectadas (pendiente normalizar)
| Plantilla | Bug | Corrección |
|---|---|---|
| Poder General (encabezado) | `{{TIPO__DE_DOCUMENTO_APODERADO}}` (doble guion) | `{{TIPO_DOCUMENTO_APODERADO}}` |
| Venta Notario Titular | `{{FECHA_EXPDICION_CERT...}}` (typo) | `{{FECHA_EXPEDICION_CERT...}}` |
| Salida del País | `{{NOTARIA_ENCARGADA}}` | `{{SELECCIONE_NOTARIO_ENCARGADO}}` |
| Salida del País | `{{VALOR_DERECHOS_NOTARIALES}}` | `{{DERECHOS_NOTARIALES}}` |

---

## MARCO LEGAL — DECRETO 960 DE 1970

Estatuto del Notariado colombiano. Artículos relevantes para EasyPro:

| Artículo | Relevancia |
|---|---|
| Art. 13-14 | Las 4 etapas: recepción, extensión, otorgamiento, autorización → nuestro flujo de 8 pasos |
| Art. 19 | Cantidades en letras Y cifras entre paréntesis → etiquetas VALOR_EN_LETRAS + VALOR_EN_NUMEROS |
| Art. 23 | Escrituras numeradas en letras y cifras → etiquetas NUMERO_ESCRITURA_EN_LETRAS + NUMEROS |
| Art. 25 | Nombre, apellido, estado civil, domicilio de comparecientes → campos de intervinientes |
| Art. 35 | Escritura leída en totalidad, otorgantes expresan asentimiento → paso revisión aprobador |
| Art. 40 | Notario autoriza en último lugar → paso firma notario es el final del flujo |
| Art. 9 | Notario responde de regularidad formal, no veracidad → cláusula de advertencia en plantillas |
| Art. 102 | Corrección post-autorización requiere instrumento separado → sustento legal del versionado |

**Pendiente para mañana:** alimentar el prompt de Gari con artículos específicos y ajustar validaciones del motor de sustitución.
...
| /dashboard/casos | Listado casos | ✅ Operativo |
| /dashboard/casos/crear | Wizard creación | ✅ Operativo |
| /dashboard/casos/{id} | Detalle caso + Gari + Previsualizador | ✅ Operativo |
| /dashboard/plantillas | Plantillas | ✅ Operativo |
| /dashboard/lotes | Lotes masivos | 🔧 Parcial |
| /dashboard/notarías | Notarías | ✅ Operativo |
| /dashboard/usuarios | Usuarios | ✅ Operativo |
...
## PENDIENTES CRÍTICOS — GO-LIVE 14 ABRIL

### Bloquean salida a producción
1. ⬜ Integrar motor sustitución `{{etiquetas}}` con plantillas recibidas
2. ⬜ Normalizar inconsistencias de etiquetas entre plantillas
3. ⬜ Deploy Railway (backend FastAPI)
4. ⬜ Deploy Vercel (frontend Next.js)
5. ⬜ Migración BD SQLite → Supabase PostgreSQL
6. ⬜ DNS subdominios helexium.co → caldas.easypro.co
7. ⬜ Variables de entorno en producción (OPENAI_API_KEY, DATABASE_URL, etc.)
8. ⬜ Validación operativa con Notaría de Caldas

### Importantes antes del go-live
- Login dinámico por notaría (imagen y colores desde BD)
- Paleta Finaktiva en dashboard/sistema autenticado
- Flujo de aprobación completo en UI (revisor → aprobador → notario)
- URLs de video testimonial para la sección casos de éxito
- Alimentar prompt Gari con artículos del Decreto 960 de 1970

### Post go-live
- Operación masiva de lotes
- Más tipos de acto en Gari (compraventa, sucesión, declaraciones)
- Integraciones externas (RNEC, firma digital certificada)
- Compraventa con propiedad horizontal (RPH)
...
### Notaría de Caldas
- Go-live: **14 de abril de 2026**
- URL objetivo: caldas.easypro.co
- Contacto: +57 310 793 2844
- Estado: pendiente configuración en Supabase

### Notaría Primera de Bello
- Rol: demo y referencia
- Contacto: +57 310 793 2844
- Estado: operativa en local
