      ...
      <section className="ep-card rounded-[2rem] p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-accent">Minutas documentales</p>
            ...
            <p className="mt-3 text-base leading-7 text-secondary">El producto ya está ordenado alrededor del proceso real: comercial, asignación de notaría, minuta, revisión, aprobación, firma y cierre.</p>
          </div>
      ...
      <section className="ep-card rounded-[2rem] p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-primary">Listado operativo de minutas</h2>
          </div>
          <div className="ep-pill inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm text-secondary"><Filter className="h-4 w-4 text-primary" />Estado, tipo de minuta, responsable y notaría</div>
        </div>
        ...
        <div className="ep-filter-panel mt-6 grid gap-4 rounded-[1.75rem] p-4 lg:grid-cols-5">
          ...
          <label className="grid gap-2 text-sm font-medium text-primary">Tipo de minuta<select value={filters.case_type ?? ""} onChange={(event) => updateFilter("case_type", event.target.value)} className="ep-select h-12 rounded-2xl px-4"><option value="">Todos</option>{filterOptions.case_types.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
          ...
          <label className="grid gap-2 text-sm font-medium text-primary">Buscar<input value={filters.q ?? ""} onChange={(event) => updateFilter("q", event.target.value)} placeholder="Acto, minuta, notaría..." className="ep-input h-12 rounded-2xl px-4" /></label>
        </div>
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_320px]">
          ...
          <aside className="space-y-4">
            <div className="rounded-[1.6rem] bg-primary p-5 text-white shadow-panel">
              ...
              <p className="mt-3 text-sm leading-6 text-white/82">El núcleo de la minuta ya diferencia diligenciamiento, revisión cliente, aprobador, notario, firmado y cierre.</p>
            </div>
            ...
            <div className="ep-card-soft rounded-[1.6rem] p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-secondary">Protocolista, Cliente, Aprobador, Notario titular y Notario suplente quedan modelados dentro de la minuta.</p>
            </div>
          </aside>
        </div>
      </section>
